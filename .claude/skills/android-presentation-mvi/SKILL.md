...
Receives the ViewModel (via `hiltViewModel()`, or `hiltViewModel<VM, VM.Factory>(creationCallback = ...)` for assisted injection) and any callbacks needed for navigation. Observes events. Passes state and `onAction` down.
